...
public class Application01 {
    public static void main(String[] args) {
         ...
         * 현실 세계와 닮아있지만 현실 세계와 다른점은,
         * 객체간의 상호 작용을 한다는 사실은 현실 세계와 통일하지만, 의인화 기법이 적용되어
         * 현실세계에서 불가능한 무생물이나 개념 같은 것들도 하나의 주제로
         * 본인의 상태를 스스로 제어하고(캡슐화) 행동하도록 한다는 것이 현실 세계와 다른 점이다.
         ...
         * 유연성 확보는 여러 곳에 적용될 수 있는 유연한 객체를 의미하며, 즉 재사용성이 높아질 수 있게 한다는 의미히다.
         * 객체의 재사용성이 증가하면 중복 작성되는 코드를 줄일 수 있으며, 오류 발생 가능성을 감소시키고 유지보수성을 증가시킨다.
         * */

        /*
         * 객체와 객체의 상호작용이란?
         * 객체와 객체는 메세지를 통해 서로 상호작용을 한다. 메세지(메소드 호출)을 보내는 쪽은 송신자 라고 하고,
         * 메세지를 수신하는 쪽을 수신자 라고 한다. 수신자는 메세지를 전달 받아 그 메세지에 해당하는 내용을 처리하는 방법을
         * 스스로 결정하고, 그 결정한 방법 대로 처리할 명령어들을 순차저긍로 기술한 것을 메소드라고 한다
         ...
         * 것을 설계하는 것을 의미한다
         * 즉, 필드보다는 메소드를 중점적으로 추상화기법을 적용하여 객체를 설계하고, 그
         * 객체를 구현하는 방법으로 설계한 대로 클래스를 작성하는 것이다. 이것을 추상화 라고 부른다.
         *
         * */
         ...
         *  1. 카레이서 클래스
         *      -속성: 자동차
         *      -행위: 시동을 걸어라 , 엑셀을 밟아라, 브레이크를 밟아라  < 플레이어가 카레이서에게 지시할 행동
         *
         * 2. 자동차 클래스
         *  - 속성 : 시동 상태 , 현재 시속
        ...
        while (true) {
            ...
            switch (no) {
                case 1:
                    carRacer.statUp();
                    break; //break 걸렸지만 while문 안에 있으므로 다시 돌아감
                case 2:
